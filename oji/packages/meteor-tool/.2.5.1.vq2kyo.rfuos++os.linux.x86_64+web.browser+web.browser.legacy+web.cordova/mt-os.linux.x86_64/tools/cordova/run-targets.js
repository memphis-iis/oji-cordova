...
class AndroidRunTarget extends CordovaRunTarget {
  ...
  checkPlatformRequirementsAndSetEnv(cordovaProject) {
    return Promise.asyncApply(() => {
      ...
      const projectPath = files.pathJoin(cordovaProject.projectRoot, 'platforms', this.platform);

      const check_reqs = require("cordova-android/lib/check_reqs"); // We can't use check_reqs.run() because that will print the values of
      // JAVA_HOME and ANDROID_HOME to stdout.


      Promise.await(check_reqs.check_all(projectPath));
    });
  }
