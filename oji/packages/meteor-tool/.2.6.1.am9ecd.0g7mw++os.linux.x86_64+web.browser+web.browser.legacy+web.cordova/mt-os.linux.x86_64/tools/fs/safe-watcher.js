...
let statOrNull, convertToOSPath, watchFile, unwatchFile, toPosixPath, pathRelative;
module.link("./files", {
  statOrNull(v) {
    statOrNull = v;
  },
...
const pathwatcher = require('pathwatcher'); // Default to prioritizing changed files, but disable that behavior (and
...
const WATCHER_CLEANUP_DELAY_MS = 30000; // Since linux doesn't have recursive file watching, nsfw has to walk the
...
let watcherLibrary = process.env.METEOR_WATCHER_LIBRARY || (process.platform === 'linux' ? 'pathwatcher' : 'nsfw'); // Pathwatcher complains (using console.error, ugh) if you try to watch
// two files with the same stat.ino number but different paths on linux, so we have
// to deduplicate files by ino.

const DEDUPLICATE_BY_INO = watcherLibrary === 'pathwatcher'; // Set METEOR_WATCH_FORCE_POLLING environment variable to a truthy value to
...
const entries = Object.create(null); // Folders that are watched recursively

let watchRoots = new Set(); // Set of paths for which a change event has been fired, watched with
...
function addWatchRoot(absPath) {
  if (watchRoots.has(absPath) || watcherLibrary !== 'nsfw' || !watcherEnabled) {
    return;
  }

  watchRoots.add(absPath); // If there already is a watcher for a parent directory, there is no need
  // to create this watcher.

  for (const path of watchRoots) {
    let relativePath = pathRelative(path, absPath);

    if (path !== absPath && !relativePath.startsWith('..') && !relativePath.startsWith('/')) {
      return;
    }
  ...
  nsfw(convertToOSPath(absPath), events => {
