...
function containsPath(path1, path2) {
  const relPath = pathRelative(path1, path2); // On Windows, if the two paths are on different drives the relative
  // path starts with /
...
function extractTarGz(buffer, destPath) {
  ...
  const startTime = +new Date(); // standardize only one way of extracting, as native ones can be tricky
...
function tryExtractWithNpmTar(buffer, tempDir) {
  ...
  return new Promise((resolve, reject) => {
    ...
    }).on('error', reject).on('finish', resolve); // write the buffer to the (gunzip|untar) pipeline; these calls
...
function createTarGzStream(dirPath) {
  ...
  const basename = pathBasename(dirPath); // Create a segment of the file path which we will look for to
  ...
  const tarStream = tar.pack(convertToOSPath(dirPath), {
    map: header => {
      header.name = "".concat(basename, "/").concat(header.name);
