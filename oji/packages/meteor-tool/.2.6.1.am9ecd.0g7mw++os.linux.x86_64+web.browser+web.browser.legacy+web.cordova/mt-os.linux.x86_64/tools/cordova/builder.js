...
class CordovaBuilder {
  constructor(projectContext, projectRoot, options) {
    _defineProperty(this, "_resolveFilenameForImages", (suppliedPath, key, tag) => {
      ...
      return "".concat(key, ".").concat(tag, ".").concat(extension);
    });
  ...
  static createCordovaServerPort(appIdentifier) {
    ...
    const appIdAsNumber = parseInt(appIdentifier, 36); // We use the appId to choose a local server port between 12000-13000.
  ...
  initalizeDefaults() {
    ...
    if (!cordovaServerPort) {
      cordovaServerPort = CordovaBuilder.createCordovaServerPort(this.projectContext.appIdentifier);
    }
    ...
    this.additionalConfiguration = {
      ...
      platform: {
        ...
        android: {
          ...
          "hostname": "localhost:".concat(cordovaServerPort),
    ...
    const setDefaultLaunchScreen = key => {
      const imageFile = files.pathJoin(launchScreensPath, "".concat(key, ".png"));
  ...
  writeConfigXmlAndCopyResources() {
    ...
    if (process.env.NODE_ENV !== 'production') {
      platformElement.android.ele("edit-config").att("file", "app/src/main/AndroidManifest.xml").att("mode", "merge").att("target", "/manifest/application").ele("application").att("android:usesCleartextTraffic", "true");
    }
  ...
  _copyImageToBuildFolderAndAppendToXmlNode(suppliedPath, newFilename, xmlElement, tag) {
    let attributes = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : {};
    const src = files.pathJoin('resources', newFilename);
    files.copyFile(files.pathResolve(this.projectContext.projectDir, suppliedPath), files.pathJoin(this.resourcesPath, newFilename)); // Set it to the xml tree

    xmlElement.ele(tag, _objectSpread({
      src
    }, attributes));
  }
  ...
  _configureAndCopyIcon(sizes, xmlElement) {
    ...
    Object.entries(sizes).forEach(_ref => {
      let [key, size] = _ref;
      ...
      this._copyImageToBuildFolderAndAppendToXmlNode(suppliedPath, filename, xmlElement, 'icon', {
        width,
        height
      });
    });
  ...
  _configureAndCopySplashImages(allowedValues, xmlElement) {
    let isIos = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;

    const appendDarkMode = function (stringValue) {
      let {
        separator = '.',
        withChar = '~'
      ...
      return "".concat(stringValue.substring(0, lastIndexOfSeparator)).concat(withChar).concat(darkModeIdentifier).concat(stringValue.substring(lastIndexOfSeparator));
    };

    Object.entries(allowedValues).forEach(_ref2 => {
      let [key, value] = _ref2;
      ...
      if (isIos) {
        if (suppliedPathDarkMode) {
          this._copyImageToBuildFolderAndAppendToXmlNode(suppliedPathDarkMode, appendDarkMode(value), xmlElement, 'splash');
        }

        this._copyImageToBuildFolderAndAppendToXmlNode(suppliedPath, value, xmlElement, 'splash');
      ...
      if (suppliedPathDarkMode) {
        this._copyImageToBuildFolderAndAppendToXmlNode(suppliedPathDarkMode, appendDarkMode(filename, {
          withChar: '_'
        }), xmlElement, 'splash', {
          density: appendDarkMode(value, {
            separator: '-',
            withChar: '-'
          })
        });
      ...
      this._copyImageToBuildFolderAndAppendToXmlNode(suppliedPath, filename, xmlElement, 'splash', {
        density: value
      });
    });
  }
  ...
  appendVersion(program, publicSettings) {
    ...
    program.versionNonRefreshable = AUTOUPDATE_VERSION || WebAppHashing.calculateClientHash(program.manifest, (type, replaceable) => type !== "css" && !replaceable, configDummy);
    program.versionReplaceable = AUTOUPDATE_VERSION || WebAppHashing.calculateClientHash(program.manifest, (_type, replaceable) => replaceable, configDummy);
  }
  ...
  generateBootstrapPage(applicationPath, program, publicSettings) {
    ...
    const hmrVersion = this.options.buildMode === 'development' ? Date.now() : undefined;
...
function createAppConfiguration(builder) {
  ...
  return {
    ...
    launchScreens: function (launchScreens) {
      const validDevices = Object.keys(splashIosKeys).concat(Object.keys(splashAndroidKeys));
