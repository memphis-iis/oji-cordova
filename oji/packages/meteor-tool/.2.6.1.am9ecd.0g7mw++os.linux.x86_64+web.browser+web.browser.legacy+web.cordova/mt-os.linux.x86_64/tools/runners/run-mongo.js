...
var runMongoShell = function (url) {
  ...
  args.push(mongoUrl.hostname + ':' + mongoUrl.port + mongoUrl.pathname); // run with rosetta on mac m1
...
function spawnMongod(mongodPath, port, dbPath, replSetName) {
  ...
  let args = [// nb: cli-test.sh and findMongoPids make strong assumptions about the
...
var launchMongo = function (options) {
  ...
  var initiateReplSetAndWaitForReady = function () {
    try {
      ...
      const client = new MongoClient("mongodb://127.0.0.1:".concat(options.port), {
        minPoolSize: 1,
        maxPoolSize: 1,
      ...
      while (!stopped) {
        ...
        if (ismaster) {
          ...
          yieldingMethod(db.admin(), 'command', _objectSpread({
