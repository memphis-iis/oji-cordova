!function (module1) {
  ...
  const EXPECTED_INVALID_PACKAGE_JSON_PATHS_TO_IGNORE = ['resolve/test/resolver/malformed_package_json']; // XXX: This is a medium-term hack, to avoid having the user set a package name
      ...
      function find(dir, depth) {
        ...
        if (inNodeModules) {
          ...
          try {
            ...
            const message = "Error reading package.json from dir \"".concat(dir, "\", this may cause important errors in your project like modules not being found. You should fix this dependency or find an alternative");

            if (EXPECTED_INVALID_PACKAGE_JSON_PATHS_TO_IGNORE.find(path => dir.includes(path))) {
            ...
            if (process.env.METEOR_WARN_ON_INVALID_PACKAGE_JSON_ERRORS) {
              console.warn(message, e); // Pretend we found no files but in reality an error happened reading this package.json
